...
var hotAreaLeftW=153, hotAreaLeftH=922, hotAreaLeftX=236, hotAreaLeftY=71;
...
function preload(){
  for (var i = start; i<finish; i++) {
    imgs[i-start]=loadImage("imgsc/cloth c_"+str(i)+".jpg");
  }
}

function setup() {
  if (windowHeight>imgHeight) {
    createCanvas(imgHeight*608/1024, imgHeight);
    r=1;
  }
  else {
    r=windowHeight/imgHeight;
    createCanvas(windowHeight*608/1024, windowHeight);
  }
  touched=0;
}
...
function draw() {
  ...
  println(imageNumber);
  n=ceil(imageNumber);
  image(imgs[n], 0, 0,imgHeight*r*608/1024,imgHeight*r);
...
function windowResized() {
  if (windowHeight>imgHeight) {
  resizeCanvas(imgHeight*608/1024, imgHeight);
  r=1;
  }
  else {
    r=windowHeight/imgHeight;
    resizeCanvas(windowHeight*608/1024, windowHeight);
  }
}
